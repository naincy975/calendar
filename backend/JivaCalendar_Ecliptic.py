...
time_periods = {
    "sun": 365.25636,
    "moon": 27.321,
    "moon_synodic": 29.5306,
}  # length of sidereal time periods in days

coord_Revati_ICRS = SkyCoord(
    ra="01h13m45.17477s",
    dec="+7d34m31.2745s",
    frame="icrs",
    equinox=Time(2000, format="jyear"),
)
coord_Revati_Ec = coord_Revati_ICRS.transform_to(
    GeocentricTrueEcliptic()
)  # Geocentric True Ecliptic coordinates of Revati (Zeta Piscium A)
...
def find_new_moon_date_before_Ec(t=Time("J2000")):
    ...
    if exact_date == 0:
        new_approx_date = datetime_to_astropy(
            astropy_to_date(approx_date) + timedelta(days=-3)
        )
...
def solve_body_time_Ec(lon, t, body, accuracy=0.01, find="previous"):
    ...
    if find.lower() == "previous":
        approx_time = astropy_to_datetime(t) - timedelta(
            days=((c - lon) % 360) / 360 * tp
        )
    if find.lower() == "next":
        approx_time = astropy_to_datetime(t) + timedelta(
            days=((lon - c) % 360) / 360 * tp
        )
    ...
    if lon > 30 and lon < 330:
        ...
        while abs(del_ang) > accuracy:
            ...
            if iter > 100:
                raise Exception(
                    "Exceeded 100 iterations inside solve_body_time_absolute_Ec"
                )
...
def get_ayanamsa(ayanamsa):
    ...
    if type(ayanamsa) in [float, int]:
        ...
        raise Exception(
            "ayanamsa must be float, int, string or astropy.coordinates.angles.Angle"
        )
...
def get_local_observations(
    location,
    t=Time("J2000"),
    sun_horizon=Angle("-0.26d"),
    moon_horizon=Angle("-0.26d"),
    find=["nearest"] * 4,
):
    ...
    if find == "in_date":
        t = datetime_to_astropy(
            astropy_to_datetime(t).replace(hour=0, minute=0, second=0)
        )
        find = ["next"] * 4
