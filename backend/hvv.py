    ...
    def HVV_test(self, accuracy=0.001):
        ...
        for i, day in enumerate(self.curr_month):
            if pt(day["tithi"]) in [9, 10]:
                list_of_days = self.return_following_days(
                    i
                )  # i.e. the given day is day 2 in the list
                [day_1, day_2, day_3, day_4, day_5, day_6, day_7, day_8, day_9] = (
                    list_of_days
                )
                vrata_day, scenario_num, tit_seq = vrata_scenario_check_1p1(
                    day_1, day_2, day_3, day_4
                )
                if vrata_day is not None:
                    vrata_day, scenario_num, tit_seq = vrata_scenario_check_1p2(
                        list_of_days, vrata_day, scenario_num, tit_seq
                    )
                    naksatra_yoga, vrata_day = naksatra_yoga_check(
                        list_of_days,
                        vrata_day,
                        accuracy=accuracy,
                        ayanamsa=self.ayanamsa,
                        mata=self.mata,
                        yama_mata=self.yama_mata,
                    )
                    parana_start, parana_end = pn.get_parana_times(
                        get_day(vrata_day, i, "vrata"),
                        get_day(vrata_day, i, "next"),
                        scenario_num,
                        naksatra_yoga,
                        accuracy=accuracy,
                        method=self.parana_method,
                    )
                    ...
                    month_vrata_list += [
                        {
                            "date": vrata_date,
                            "type": scenario_num,
                            "naksatra yoga": naksatra_yoga,
                            "info": create_string(tit_seq, vrata_day),
...
def vrata_scenario_check_1p1(day_1, day_2, day_3, day_4, dawn_duration=96):
    ...
    _, titd, _, _ = jcf.get_dawn_info(
        sunrise=day_2["sunrise"], dawn_duration=dawn_duration
    )
...
def vrata_scenario_check_1p2(l, vrata_day, scenario_num, tit_seq):
    ...
    tit_seq_full = [t for t in tit_seq] + list(
        map(pt, tithis[4:])
    )  # This is only for returning. Had to convert
    ...
    if len(paksa_end_tithis) == 2:
        if scenario_num[0:3] == "hvv" and tit_seq[vrata_day] == 11:
            vrata_day = (
                tit_seq[2:].index(12) + 2
            )  # This selects the first occurence of dvadasi, if dvadasi rules 2 days
            scenario_num = "pvm1"  # pv stands for pakśa-vardhinī
            return vrata_day, scenario_num, tit_seq_full
        if scenario_num[0:3] in ["unm", "tsp", "vnj"]:
            scenario_num = (
                scenario_num[0] + "pv1"
            )  # i.e. upv1, tpv1 or vpv1. "pv" stands for pakśa-vardhini
            return vrata_day, scenario_num, tit_seq_full
    ...
    if mata == "dina-mana":
        for i, nak_ in enumerate([3, 6, 7, 21]):
            if nak_sunrise1 == nak_:  # Rohiṇī
                if start_at_sunrise:
                    return dvadasi_names[i] + "1", day_1_num
                nak_start = jcf.get_sankramana_time_Ec(
                    t=sunrise1,
                    body="moon",
                    accuracy=accuracy,
                    ayanamsa=ayanamsa,
                ...
                nak_end = jcf.get_sankramana_time_Ec(
                    t=sunrise1,
                    body="moon",
                    accuracy=accuracy,
                    ayanamsa=ayanamsa,
...
def convert_timezones(data, offset):
    ...
    for d in data:
        d["parana"] = (
            d["parana"][0].astimezone(timezone),
            d["parana"][1].astimezone(timezone),
        )
        new_data += [d]
    return new_data
...
def get_year_vrata(
    ...
    next_yr_1st_month = jcf.get_month_data(
        year=year + 1,
        month=1,
        latitude=27.5650,
        longitude=77.6593,
