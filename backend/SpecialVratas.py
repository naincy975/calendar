...
def main(
    ...
    rn = get_rama_navami(
        data, hvv_date_list, accuracy=accuracy, timezone_offset=timezone_offset
    )
    kj = get_krsna_janmastami(
        data, accuracy=accuracy, timezone_offset=timezone_offset, ayanamsa=ayanamsa
    )
...
def get_siva_ratri(data, muhurta=48, accuracy=0.001, timezone_offset=None):
    ...
    for i, day in enumerate(data):
        ...
        if tit_seq == [28, 29, 30]:
            ...
            _, t_e = jcf.get_tithi_start_end_Ec(
                t=day2_sunrise,
                accuracy=accuracy,
                get_start=False,
                which_tithi="current",
        ...
        if tit_seq == [27, 29, 30]:
            ...
            if t_e - day2_sunrise < 2 * muhurta:
                print(
                    "WARNING! Something's off. 28th is ksaya but 29th doesn't span even 2 muhurtas."
                )
        ...
        if scenario is not None:
            ...
            all_vratas_list += [
                (vrata_day["gregorian_date"], parana_start, parana_end, scenario)
            ]
...
def get_krsna_janmastami(
    ...
    for i, day in enumerate(data):
        ...
        tit_seq = [
            d["tithi"] - 15 for d in day_seq[0:-1]
        ]  # converting so that we can call krsna paksa astami as 8th instead of 23rd
        # masa = day['masa']
        ...
        if tit_seq == [7, 8, 8] or tit_seq == [6, 8, 8]:
            ...
            d2_dow = (
                d2["gregorian_date"].strftime("%A").lower()
            )  # this is day of the week as a lower case string
            ...
            if (
                d2["moon_naksatra"] != "rohini"
                and d3_dow not in ["monday", "wednesday"]
                and d3["moon_naksatra"] != "rohini"
            ):
                if nak != "rohini":
                    scenario = 5  # both scenarios 5 and 6 are included here.
                    vrata_index = 1
            if (
                d2_dow not in ["monday", "wednesday"]
                and d2["moon_naksatra"] != "rohini"
                and d3_dow in ["monday", "wednesday"]
                and d3["moon_naksatra"] != "rohini"
            ):
                if nak != "rohini":
                    scenario = 7
        ...
        if scenario == 4:
            print(
                "WARNING! Scenario 4 is valid but nothing else fits the description for krsna janmastami"
            )
        ...
        if scenario == 1 or scenario >= 4:
            ...
            roh_s, _ = jcf.get_sankramana_time_Ec(
                t=sunrise,
                body="moon",
                find="nearest",
                which_nak="rohini",
            ...
            tit, tit_s, tit_e = jcf.get_tithi_start_end_Ec(
                t=sunrise, accuracy=accuracy, get_start=True, which_tithi="current"
            )
            if tit != 23:
                print(
                    "WARNING! Scenario 1 or >=4 for janmastami but the vrata day tithi is not astami. Tithi is",
                    tit,
                )

            if roh_s > tit_s and roh_s < tit_e:
                # i.e. rohini is present at some point in the tithi
                roh_e, _ = jcf.get_sankramana_time_Ec(
                    t=sunrise,
                    body="moon",
                    find="nearest",
                    which_nak="rohini",
                ...
                if tit_e < next_sunrise and roh_e < next_sunrise:
                    parana_start = next_sunrise
                    next_tit, next_tit_e = jcf.get_tithi_start_end_Ec(
                        t=next_sunrise,
                        accuracy=accuracy,
                        get_start=False,
                        which_tithi="current",
                    ...
                    if (
                        next_tit == 24
                    ):  # Checking to see if the tithi on next sunrise (after the vrata day) is navmi
                ...
                _, tit_e = jcf.get_tithi_start_end_Ec(
                    t=vrata_day["sunrise"],
                    accuracy=accuracy,
                    get_start=False,
                    which_tithi="current",
