...
def get_parana_times(
    ...
    if scenario_num in ["vnj1"] and nak_yoga_scenario is None:
        choice = "vnj"
    if scenario_num[0:3] == "hvv" and nak_yoga_scenario in [
        "jyt1",
        "vjy1",
        "jyt2",
        "vjy2",
    ...
    if choice is None:
        raise ValueError(
            f"No pāraṇa scenario fits the vrata. Vrata Scenario is {scenario_num} and nakṣatra yoga {nak_yoga_scenario}"
        )
...
def standard(vrata_day, next_day, accuracy=0.01, method="common_sense"):
    ...
    tit, ddsi_start, ddsi_end = jcf.get_tithi_start_end_Ec(
        t=sunrise, accuracy=accuracy, get_start=True
    )  # dvadasi start & end times
    tit = pt(tit)
    if tit != 12:
        print(
            "WARNING! Using standard pāraṇa calculation but tithi next day is not dvādaśī. Tithi is",
            tit,
        )
...
def alternate(vrata_day, next_day, accuracy=0.01):
    ...
    if vrata_day["tithi"] < 15:
        ...
        trayodasi = 28
    sunrise, sunset = next_day["sunrise"], next_day["sunset"]
    tit, ddsi_end, tdsi_end = jcf.get_tithi_start_end_Ec(
        t=sunrise,
        accuracy=accuracy,
        which_tithi=trayodasi,
        find="nearest",
        get_start=True,
    )  # dvādaśī ends when trayodaśī starts
    tit = pt(tit)
...
def vanjuli(vrata_day, next_day, accuracy=0.01):
    ...
    if vrata_day["tithi"] < 15:
        ...
        dvadasi = 27
    sunrise, sunset = next_day["sunrise"], next_day["sunset"]
    tit, ddsi_end = jcf.get_tithi_start_end_Ec(
        t=sunrise,
        accuracy=accuracy,
        get_start=False,
        which_tithi=dvadasi,
        find="nearest",
    )  # dvādaśī's start & end times
    tit = pt(tit)
...
def jayanti_vijaya(vrata_day, next_day, accuracy=0.01):
    ...
    nak_end_r, nak_num_r = jcf.get_sankramana_time_Ec(
        t=sunrise,
        body="moon",
        accuracy=accuracy,
        find="nearest",
        which_nak=3,
        start_end="end",
    )  # Loading Rohini end time
    nak_end_s, nak_num_s = jcf.get_sankramana_time_Ec(
        t=sunrise,
        body="moon",
        accuracy=accuracy,
        find="nearest",
...
def jaya_papa_nasini(vrata_day, next_day, accuracy=0.01):
    ...
    nak_end_1, nak_num_1 = jcf.get_sankramana_time_Ec(
        t=sunrise,
        body="moon",
        accuracy=accuracy,
        find="nearest",
        which_nak=6,
        start_end="end",
    )  # Loading Punaravasu end time
    nak_end_2, nak_num_2 = jcf.get_sankramana_time_Ec(
        t=sunrise,
        body="moon",
        accuracy=accuracy,
        find="nearest",
